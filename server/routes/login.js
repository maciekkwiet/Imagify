...
router.post('/', async (req, res) => {
  ...
  //   console.log(error);
  if (error) return res.status(400).json({ text: 'error 400' });
  ...
  if (!user) return res.status(400).json({ text: 'Invalid email or password' });

  const validPassword = await bcrypt.compare(req.body.password, user.password);
  if (!validPassword) return res.status(400).json({ text: 'error 400' });
